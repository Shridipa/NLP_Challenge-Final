...
def run_pipeline(user_query, history=None):
    ...
    if history:
        # Limit history processing to last 5 messages for performance
    ...
    # Check if we should adopt the previous intent (either action or ask)
    ...
    if (previous_action_intent or (history and len(history) > 0)) and intent == "other" and not is_informational_query:
        # Determine previous intent from history if not explicit
        prev_intent = previous_action_intent
        if not prev_intent and history:
            # Re-detect intent of last user message
        ...
        if not os.path.exists(index_file) or not os.path.exists(mapping_file):
            ...
            if " and " in user_query.lower() or "," in user_query:
                ...
                for qp in query_parts:
                    part_chunks = retrieve_chunks(qp, index_file, mapping_file, k=4, boost_keywords=boost_kws, section_filter=target_section)
                    if part_chunks:
                ...
                retrieved_chunks = retrieve_chunks(user_query, index_file, mapping_file, k=5, boost_keywords=boost_kws, section_filter=target_section)
            
        if retrieved_chunks:
            # Map the top-1 re-ranker score (usually -10 to 10) to a 0-1 confidence score
            # A score of 0.0 or higher is generally very strong. -2.0 is still quite relevant.
            top_score = retrieved_chunks[0]['score']
            
            # Simple sigmoid-like normalization for Logit scores
            retrieval_score = 1.0 / (1.0 + pow(2.718, -(top_score + 1.0)))
            
            # Entity validation - keep it but make it less restrictive
            ...
            if check_list:
                ...
                # Search across all retrieved chunks, not just top 2
                ...
                # If we have keywords but none are in the chunks, penalize but don't kill the score immediately
